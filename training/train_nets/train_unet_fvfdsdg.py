...
from deeplearning.models.unet_fvfdsdg import UNetFVFDSDG, Projector  
...
from deeplearning.datasets.utils.transform import collate_fn_tr_styleaug, collate_fn_ts  
from deeplearning.utils.lr import adjust_learning_rate  
from deeplearning.utils.metrics.metrics import get_hard_dice, get_hard_precision, get_hard_recall, get_hard_f1  
...
def train(args):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iter, batch in enumerate(tr_dataloader):
            ...
            train_retina_dice_list.append(get_hard_dice(output_sigmoid[:, 0].cpu(), (seg[:, 0] > 0).cpu() * 1.0))
            del seg
        mean_tr_loss = np.mean(train_loss_list)
        mean_content_loss = np.mean(content_loss_list)
        mean_style_loss = np.mean(style_loss_list)
        mean_tr_retina_dice = np.mean(train_retina_dice_list)
        ...
        writer.add_scalar("Train Scalars/Retina Dice", mean_tr_retina_dice, epoch)
        print('  Tr loss: {}\n'
              '  Tr retina dice: {}'.format(mean_tr_loss, mean_tr_retina_dice))

        val_loss_list = list()
        val_retina_dice_list = list()
        with torch.no_grad():
            model.eval()
            for iter, batch in enumerate(ts_dataloader):
                ...
                seg = torch.from_numpy(batch['seg']).cuda().to(dtype=torch.float32)
                output = model(data, tau=tau)
                val_loss = criterion(output[:, 0], (seg[:, 0] > 0) * 1.0)  # Only consider retina
                val_loss_list.append(val_loss.detach().cpu().numpy())
                output_sigmoid = torch.sigmoid(output)
                val_retina_dice_list.append(get_hard_dice(output_sigmoid[:, 0].cpu(), (seg[:, 0] > 0).cpu() * 1.0))
        mean_val_loss = np.mean(val_loss_list)
        mean_val_retina_dice = np.mean(val_retina_dice_list)
        writer.add_scalar("Val Scalars/Val Loss", mean_val_loss, epoch)
        writer.add_scalar("Val Scalars/Val Retina Dice", mean_val_retina_dice, epoch)
        writer.add_image('Val/Input', make_grid(data[:10], 10, normalize=True), epoch)
        writer.add_image('Val/Output_vessel', make_grid(output_sigmoid[:10, 0][:, np.newaxis], 10, normalize=True), epoch)
        ...
        print('  Val loss: {}\n'
              '  Val retina dice: {}'.format(mean_val_loss, mean_val_retina_dice))
    ...
    # inference
    from deeplearning.inference.inference_nets.inference_unet_fvfdsdg import inference
